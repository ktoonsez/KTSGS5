...
int gsimple_laziness = 4;
int gsimple_ramp_up_threshold = 5000;
static int laziness = 0;
			...
			struct devfreq_msm_adreno_tz_data *priv)
{
	...
	/* it's currently busy */
	if (priv->bin.busy_time > gsimple_ramp_up_threshold) {
		if (level == 0)
...
static struct devfreq_governor msm_kt_simple = {
	.name = "kt-simple",
	.get_target_freq = tz_get_target_freq,
	.event_handler = tz_handler,
};

static int __init msm_kt_simple_init(void)
{
	return devfreq_add_governor(&msm_kt_simple);
}
subsys_initcall(msm_kt_simple_init);

static void __exit msm_kt_simple_exit(void)
{
	int ret;
	ret = devfreq_remove_governor(&msm_kt_simple);
	if (ret)
...
module_exit(msm_kt_simple_exit);
