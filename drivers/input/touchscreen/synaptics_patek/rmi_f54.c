...
static void timeout_set_status(struct work_struct *work)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "[synaptics] %s\n", __func__);
		...
		int update_type)
{
	...
	case 1:
		...
		break;
	case 2:
		retval = synaptics_load_fw_from_kernel(rmi4_data, rmi4_data->fac_firmware_name);
		break;
	default:
...
static void get_fw_ver_bin(void)
{
	...
	set_default_result(data);
	sprintf(data->cmd_buff, "SY%04X%02X",
...
static void get_fac_fw_ver_bin(void)
{
	...
	retval = request_firmware(&fw_entry, rmi4_data->fac_firmware_name,
			&rmi4_data->i2c_client->dev);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
...
static void hover_enable(void)
{
	...
	if (data->cmd_param[0] < 0 || data->cmd_param[0] > 1) {
		...
		rmi4_data->hover_called = true;
		rmi4_data->hover_ic = rmi4_data->flip_status;
...
static void switch_sel(void)
{
	...
	sel_status = data->cmd_param[0];
	dev_info(&rmi4_data->i2c_client->dev, "%s : %d\n", __func__, sel_status);
	sprintf(data->cmd_buff, "switch to %s", sel_status ? "TSP_SUB" : "TSP_MAIN");

	if(rmi4_data->tsp_probe && (sel_status == TSP_MAIN || sel_status == TSP_SUB)){
		gpio_set_value(rmi4_data->dt_data->tsp_sel, sel_status);
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "OK");
		data->cmd_state = CMD_STATUS_OK;
	} else {
		snprintf(data->cmd_buff, sizeof(data->cmd_buff), "NG");
