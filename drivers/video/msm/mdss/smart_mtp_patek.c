...
static int gradation_offset_EA8061V_revB[][9] = {
...
static int rgb_offset_EA8061V_revB[][RGB_COMPENSATION] = {
...
static int get_base_luminance(struct SMART_DIM *pSmart)
{
	...
	if (id3 == EA8061V_REV_B)
		memcpy(base_luminance, base_luminance_revB, sizeof(base_luminance_revB));
	else
		memcpy(base_luminance, base_luminance_revB, sizeof(base_luminance_revB));
...
{
	if(id3 == EA8061V_REV_B)
		return gradation_offset_EA8061V_revB[table_index][index];
	else
		return gradation_offset_EA8061V_revB[table_index][index];
}
...
{
	if(id3 == EA8061V_REV_B)
		return rgb_offset_EA8061V_revB[table_index][index];
	else
		return rgb_offset_EA8061V_revB[table_index][index];
}
